# Copyright 2004 by Vinay Sajip. All Rights Reserved.
#
...
Development and testing has only been carried out (so far) on Python 2.3.4.
See the test module (test_config.py) included in the
U{distribution<http://www.red-dove.com/python_config.html#download|_blank>}.
...
@copyright: Copyright (C) 2004 Vinay Sajip. All Rights Reserved.
...
__status__  = "alpha"
__version__ = "0.3.4"
__date__    = "11 November 2004"

from types import FloatType, StringType, UnicodeType
...
class ConfigInputStream:
    ...
    def __init__(self, stream):
        ...
        @param stream: The underlying stream to be read.
        ...
        if signature[:3] == codecs.BOM_UTF8:
            ...
            used = 2
            encoding = 'utf-16be'
        #elif signature == codecs.BOM_UTF32_LE:
        #    encoding = 'utf-32le'
...
class ConfigReader:
    ...
    def parseKeyValuePair(self, parent):
        ...
        if tt == WORD:
            ...
            raise ConfigFormatError("%s: expecting word or string, found %r" % (self.location(), tv))
        self.token = self.getToken()
        ...
        if tt not in [EOF, WORD, STRING, RCURLY, COMMA]:
            raise ConfigFormatError("%s: expecting one of EOF, WORD, STRING, RCURLY, COMMA, found %r" % (self.location(), self.token[1]))
        if tt == COMMA:
            self.token = self.getToken()
...
#            reader = ConfigReader(rv)
#            global streamOpener
#            if streamOpener is None:
#                streamOpener = defaultStreamOpener
#            f = streamOpener(eval(fn))
#            reader.load(f, parent, suffix)
#            f.close()
        return rv
...
class ConfigMerger:
    ...
    def handleMismatch(self, obj1, obj2):
        ...
        @type obj2: any
        """
        raiseConfigError("unable to merge %r with %r" % (obj1, obj2))
