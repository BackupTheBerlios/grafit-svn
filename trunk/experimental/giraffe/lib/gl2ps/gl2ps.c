...
void gl2psMsg(int level, char *fmt, ...){
  va_list args;
...
void gl2psListRealloc(GL2PSlist *list, int n){
  if(n <= 0) return;
  if(!list->array){
...
GL2PSlist *gl2psListCreate(int n, int incr, int size){
  GL2PSlist *list;
...
void *gl2psListPointer(GL2PSlist *list, int index){
...
void gl2psListRead(GL2PSlist *list, int index, void *data){
  if((index < 0) || (index >= list->n)){
...
void gl2psCreateSplitPrimitive(GL2PSprimitive *parent, GL2PSplane plane,
                               GL2PSprimitive *child, int numverts,
...
void gl2psAddIndex(int *index0, int *index1, int *nb, 
                   int i, int j){
  int k;
...
int gl2psGetIndex(int i, int num){
  return(i < num-1) ? i+1 : 0;
}

int gl2psTestSplitPrimitive(GL2PSprimitive *prim, GL2PSplane plane){
...
int gl2psSplitPrimitive(GL2PSprimitive *prim, GL2PSplane plane, 
                          GL2PSprimitive **front, GL2PSprimitive **back){
  int i, j, in=0, out=0, in0[5], in1[5], out0[5], out1[5];
  int type;
  GLfloat d[5]; 
...
int gl2psFindRoot(GL2PSlist *primitives, GL2PSprimitive **root){
...
int gl2psGetPlaneFromPoints(GL2PSxyz a, GL2PSxyz b, GL2PSplane plane){  
  GLfloat n; 
...
int gl2psCheckPoint(GL2PSxyz point, GL2PSplane plane){
  GLfloat pt_dis;
...
int gl2psCheckPrimitive(GL2PSprimitive *prim, GL2PSplane plane){
  int i;
  int pos;
...
int gl2psAddInBspImageTree(GL2PSprimitive *prim, GL2PSbsptree2d **tree){
  int ret = 0;
...
void gl2psAddPolyPrimitive(int type, int numverts, 
...
int gl2psGetVertex(GL2PSvertex *v, GLfloat *p){
  int i;
...
void gl2psParseFeedbackBuffer(int used){
  char flag, dash = 0;
  int boundary;
  int i, count, v, vtot, offset = 0;
...
void gl2psPrintPostScriptBeginViewport(int viewport[4]){
  int index;
  GLfloat rgba[4];
...
int gl2psPrintPostScriptEndViewport(void){
  int res;
...
void gl2psPrintTeXHeader(void){
  ...
  fprintf(gl2ps->stream, 
          "\\newlength{\\glpsunit}\n"
          "\\setlength{\\glpsunit}{%s} \\divide \\glpsunit by %d\n"
          "\\setlength{\\unitlength}{\\glpsunit}\n"
          "\\begin{center}\n"
          "\\begin{picture}(0,0)\n"
          "\\includegraphics[%s]{%s}\n"
          "\\end{picture}%%\n"
          "%s\\begin{picture}(%d,%d)(0,0)\n",
          (gl2ps->options & GL2PS_LANDSCAPE) ? "\\textheight" : "\\textwidth",
          (int)gl2ps->viewport[2],
         (gl2ps->options & GL2PS_LANDSCAPE) ? "height=\\textheight" : "width=\\textwidth",
          name, 
...
void gl2psPrintTeXPrimitive(void *data){
  ...
  switch(prim->type){
  case GL2PS_TEXT :
    fprintf(gl2ps->stream, "\\fontsize{%d\\glpsunit}{0}\n\\selectfont", 
...
void gl2psPrintPDFBeginViewport(int viewport[4]){
  int offs;
  int index;
  GLfloat rgba[4];
  int x = viewport[0], y = viewport[1], w = viewport[2], h = viewport[3];
...
GL2PSDLL_API int gl2psTextOpt(const char *str, const char *fontname, int fontsize,
                                int alignment, GL2PSrgba rgba){
  GLfloat pos[4];
  GL2PSprimitive *prim;
  GLboolean valid;
...
GL2PSDLL_API int gl2psText(const char *str, const char *fontname, int fontsize){
  return gl2psTextOpt(str, fontname, fontsize, GL2PS_TEXT_BL, NULL);
}

GL2PSDLL_API int gl2psDrawPixels(GLsizei width, GLsizei height,
                                   int xorig, int yorig,
...
GL2PSDLL_API int gl2psPointSize(GLfloat value){
  if(!gl2ps) return GL2PS_UNINITIALIZED;
...
GL2PSDLL_API int gl2psLineWidth(GLfloat value){
  if(!gl2ps) return GL2PS_UNINITIALIZED;
